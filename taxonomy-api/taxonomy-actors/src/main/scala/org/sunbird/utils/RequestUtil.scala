...
object RequestUtil {
	private val SYSTEM_UPDATE_ALLOWED_FRAMEWORK_STATUS = List("Live", "Unlisted")
	...
	def validateRequest(request: Request): Unit = {
		...
		if (request.get("status") != null && SYSTEM_UPDATE_ALLOWED_FRAMEWORK_STATUS.contains(request.get("status").asInstanceOf[String]))
			throw new ClientException(ErrorCodes.ERR_BAD_REQUEST.name(), s"Cannot update framework status to : ${SYSTEM_UPDATE_ALLOWED_FRAMEWORK_STATUS.mkString("[", ", ", "]")}.")
